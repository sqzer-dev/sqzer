...
1. [x] `clap` derive in `sqzer-cli` with the grammar above; `-h` / `--help` tiering; `--color` and `NO_COLOR`. (`--max-width` and `--max-height` arrived with the resize stage, `--keep-metadata` with the EXIF and XMP passthrough. `--resize`, `--filter` and `--threads` are not in the grammar yet: the stage only scales down with Lanczos3, and there is no encoder thread budget. They land with those pipeline pieces rather than as flags that do nothing.)
