...
1. [x] `clap` derive in `sqzer-cli` with the grammar above; `-h` / `--help` tiering; `--color` and `NO_COLOR`. (`--max-width`, `--max-height`, `--resize`, `--filter`, `--keep-metadata` and `--threads` are not in the grammar yet: the library has no resize stage, no EXIF/XMP passthrough and no encoder thread budget. They land with those pipeline pieces rather than as flags that do nothing.)
