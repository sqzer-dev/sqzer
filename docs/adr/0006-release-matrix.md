...
3. [ ] Before the first tag: create the `sqzer-dev/homebrew-tap` repository (empty, public), mint `HOMEBREW_TAP_TOKEN` with contents write on it and add the secret to this repository, then rehearse once with `pr-run-mode = "upload"` on a branch and check `sqzer --list-codecs` from each of the six archives against the table in section 2.
4. [ ] Tag `v0.0.1` once `CHANGELOG.md` has a heading for it; `dist` takes the release notes from there.
5. [ ] Make the `EncoderUnavailable` hint target-aware, so the musl archive does not send its user to the releases page.
6. [ ] Try `cargo-zigbuild` for the musl C++ backends in a separate branch; if it works, move `native-jxl` and `native-jpegli` onto musl in `sqzer-native-tier`, `native_set.rs` and `ci.yml` together.
