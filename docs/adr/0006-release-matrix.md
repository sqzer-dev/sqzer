...
4. [ ] Rehearse once with `pr-run-mode = "upload"` on a branch and check `sqzer --list-codecs` from each of the six archives against the table in section 2.
5. [ ] Tag `v0.0.1` once `CHANGELOG.md` has a heading for it; `dist` takes the release notes from there.
6. [ ] Make the `EncoderUnavailable` hint target-aware, so the musl archive does not send its user to the releases page.
